...
| Wire routing | ⚠ **Being rebuilt — see `docs/superpowers/specs/2026-05-28-wire-routing-rules-design.md`.** Current engine (`src/layout/wires/`): a topology-template router (straight / Z / L / U / detour per edge-pair) with per-bundle lane allocation + sibling stamping. No A*/grid (that was the v1 router, since removed). Known-bad: wires pierce shapes, detour canvas-wide, and run parallel-within-gap. Rebuild is validator-first per that spec. |
