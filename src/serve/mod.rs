//! Local dev-server for `plume serve FILE`. Hand-rolled HTTP/1.1 over std::net,
...
pub fn serve(file: PathBuf, port: u16, opts: Options) -> std::io::Result<()> {
    ...
    eprintln!(
        "plume serve: {} → http://{}/  (Ctrl-C to stop)",
        file.display(),
        addr
    );
...
fn serve_events(stream: &mut TcpStream, state: &State) -> std::io::Result<()> {
    ...
    let mut last_gen = *state.gen.lock().unwrap();
    let mut last_heartbeat = Instant::now();
    loop {
        let current = *state.gen.lock().unwrap();
        if current != last_gen {
            last_gen = current;
