...
/// of attrs (style refs are flattened out, applied in source order).
pub struct StyleTable {
    expanded: HashMap<String, Vec<ResolvedAttr>>,
}
...
impl StyleTable {
    pub fn build(defs: &[StyleDef], vars: &VarTable) -> Result<Self, Error> {
        ...
        let mut expanded: HashMap<String, Vec<ResolvedAttr>> = HashMap::new();
        for def in defs {
    ...
    pub fn lookup(&self, name: &str) -> Option<&[ResolvedAttr]> {
        self.expanded.get(name).map(Vec::as_slice)
    }
}
