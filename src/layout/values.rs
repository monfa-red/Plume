...
pub fn as_number(value: &ResolvedValue, span: Span) -> Result<f64, Error> {
    match value {
        ...
        ResolvedValue::LiveVar { name, .. } => Err(Error::at(
            span,
            format!(
                "var(--plume-{}) is a visual variable; layout attrs require a number",
                name
            ),
        )),
        other => Err(Error::at(
...
pub fn as_number_tuple(value: &ResolvedValue, span: Span) -> Result<Vec<f64>, Error> {
    match value {
        ...
        ResolvedValue::LiveVar { name, .. } => Err(Error::at(
            span,
            format!(
                "var(--plume-{}) is a visual variable; layout attrs require a number or tuple",
                name
            ),
        )),
        other => Err(Error::at(
...
pub fn layout_var(vars: &VarTable, name: &str) -> Option<f64> {
    let entry = vars.get(name)?;
    extract_baked_number(&entry.value)
}

fn extract_baked_number(value: &ResolvedValue) -> Option<f64> {
    match value {
        ResolvedValue::Number(n) => Some(*n),
        ResolvedValue::LiveVar { baked: Some(b), .. } => extract_baked_number(b),
        _ => None,
    }
}
