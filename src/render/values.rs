...
///
/// Live mode emits `var(--plume-name)`; bake mode follows the var indirectly
...
/// Like `attr_str` but the fallback is the plume CSS variable named
...
pub fn attr_num(attrs: &AttrMap, name: &str) -> Option<f64> {
        ...
        ResolvedValue::List(items) => {
            ...
            for item in items {
                if let ResolvedValue::Tuple(t) = item {
                    if t.len() == 2 {
                        let x = extract_num(&t[0])?;
                        let y = extract_num(&t[1])?;
                        out.push((x, y));
                        continue;
                    }
                }
...
pub fn build_classes(
    ...
    let mut classes = vec!["plume-node".to_string()];
    for name in type_chain {
        classes.push(format!("plume-shape-{}", name));
    }
    classes.push(format!("plume-shape-{}", primitive_kind));
    for name in applied_styles {
        classes.push(format!("plume-style-{}", name));
    }
    classes.join(" ")
}
