...
- [x] Test `lone_target_stays_centred_when_a_fixed_source_is_out_of_its_span`: a tall
...
- [x] Replace the `aligned_axis_overlaps` block with the three-case logic above
  (helper `usable(rect, side, clearance) -> (lo, hi)`; reuse `port_at`). Remove the
  now-unused `slide`.
- [x] Existing `lone_facing_wire_slides_to_a_straight_shot`,
  `forced_opposite_but_offset_sides_keep_centred_ports`,
  `lone_wire_keeps_the_side_midpoint` still pass.
- [x] Render `wires_realistic` (row): `cat->bowl` connects to bowl's left midpoint.
  Verify via the SVG path (bowl port y ≈ bowl centre y).
- [x] clippy/fmt; accept the changed snapshots after PNG-check; commit.
...
- [x] Test `score_counts_turns_and_length`: a hand-built `RoutedWire` list with known
  bends/length → expected tuple. Watch it fail (module absent).
- [x] Create `score.rs` with `Score` + `score()`; re-export from `mod.rs`.
- [x] Replace `mod::quality` calls with `score::score`; delete `mod::quality` + the
  local `Score` alias.
- [x] `cargo test`; clippy/fmt; commit. (No snapshot change — same numbers.)
...
- [x] Move the existing tests that assert *slot* behaviour to drive `assign`; keep
...
- [x] Test `candidates_exclude_the_back_side_and_pin_forced`.
- [x] Test `seed_matches_geometry_on_a_plain_pair`.
...
- [x] Test `proxy_prefers_fewer_turns_between_two_side_choices` on a constructed pair.
...
- [x] Test `search_unifies_a_two_way_convergence` (the old roof case → 0 crossings).
- [x] Test `search_partitions_a_three_way_convergence` (cat+bird+water → roof: cat
  one side, bird+water another, 0 crossings, fewer turns than all-on-one-side).
- [x] Test `search_is_a_no_op_when_seed_is_optimal` (a clean pair → seed unchanged).
...
- [x] Test `group_unify_escapes_a_local_minimum` (a case single-flips can't reach).
...
- [x] `cargo test` green; the convergence integration test
  (`realistic_convergence_crossings_are_minimised`) still passes (now via `search`).
- [x] `compile_is_byte_identical` green.
- [x] clippy/fmt; review + accept snapshots after PNG-check; commit.
...
- [x] `wires_star`: one hub with ≥6 spokes in every direction → all reachable,
  even slots, 0 hard/B2n.
- [x] `wires_grid`: 3×3 nodes with row+column wires → clean, minimal crossings.
- [x] `wires_nested`: groups within groups, cross-boundary wires → passable
  ancestors respected, 0 hard/B2n.
- [x] `no_sample_breaks_a_hard_guarantee` + `baseline_contract_report` cover all new
  scenes. PNG-check each.
- [x] Commit (samples + snapshots).
...
- [x] `PLAN.md`: replace the convergence section with the side-search; mark issues
  1–5 resolved; note residuals.
- [x] `WIRING.md`: confirm C1 wording covers perpendicular eligibility (it already
  says "the side giving the best route"); no other rule edits.
- [x] Final: `cargo test`, clippy `-D warnings`, `fmt --check`, determinism all
