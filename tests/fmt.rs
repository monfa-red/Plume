...
fn fmt_every_sample_is_idempotent() {
    ...
    for entry in std::fs::read_dir(&samples_dir).expect("read samples dir") {
        ...
        let pass1 = plume::format_source(&src).expect("fmt pass 1");
        let pass2 = plume::format_source(&pass1).expect("fmt pass 2");
        if pass1 != pass2 {
...
fn formatted_output_resolves_identically() {
    ...
    for entry in std::fs::read_dir(&samples_dir).expect("read samples dir") {
        ...
        let src = std::fs::read_to_string(&path).expect("read sample");
        let formatted = plume::format_source(&src).expect("format");

        let svg_orig = plume::compile_str_with(&src, &opts).expect("compile original");
        let svg_fmt = plume::compile_str_with(&formatted, &opts).expect("compile formatted");
        if svg_orig != svg_fmt {
            failures.push(name);
        }
    }
