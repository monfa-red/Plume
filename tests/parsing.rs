...
fn endpoint_dotpath_navigates_into_groups() {
    plume::check_parse("garden |group| { frog |rect| }\ngarden.frog -> outside\n")
        .expect("dot-path endpoint");
}

#[test]
fn endpoint_side_suffix_parses() {
    plume::check_parse("cat |rect|\ndog |rect|\ncat.r -> dog.l\n").expect("side suffix");
}

#[test]
fn fan_out_with_ampersand_parses() {
    plume::check_parse("cat -> dog & bird\n").expect("fan-out");
    plume::check_parse("fox & owl -> mouse\n").expect("fan-in");
    plume::check_parse("a & b -> c & d\n").expect("cartesian fan");
}
